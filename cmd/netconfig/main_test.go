...
func Test_Command_Validation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "unknown source", input: "source: invalid", arguments: []string{"-once"}, message: "source must be netplan or native"},
		{name: "malformed topology", input: "source: native\nnative: {ethernets: {eth0: {}}}", arguments: []string{"-check"}, message: "expected a base KNI"},
		{name: "unknown flag", input: "source: native\nnative: {}", arguments: []string{"-unknown"}, message: "flag provided but not defined"},
		{name: "positional argument", input: "source: native\nnative: {}", arguments: []string{"unexpected"}, message: "unexpected positional"},
	} {
...
// input, orderly shutdown and restart reconciliation through the real executable.
...
`, mtu)
	...
	require.True(t, addresses[0].Equal(*foreign) || addresses[1].Equal(*foreign))
	vlan, err = handle.LinkByName("vlan8")
	require.NoError(t, err)
	require.Equal(t, originalVLANIndex, vlan.Attrs().Index)
